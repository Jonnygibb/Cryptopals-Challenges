...
fn main() {
    ...
    let bytes_2 = match decode_hex(hex_string_2) {
        Ok(decoded_hex) => decoded_hex,
        Err(error) => {
            println!("Error {}", error);
            return;
    ...
    // Zip both byte slices together applying xor operator byte by byte.
    let xor_result: Vec<u8> = bytes_1.iter().zip(bytes_2.iter()).map(|(&a, &b)| a ^ b).collect();
